var socket = io();
var params = getQueryParams() || { name: "", room: "General" };
...
$("#room-details").append(
  "<p>Welcome to the " + params.room + " chat room</p>"
);
...
socket.on("message", function (message) {
  ...
  $messages.append(
    "<p class='col-xs-12 col-sm-8 col-sm-offset-2 message " +
      messageType +
      "'><strong>" +
      message.name +
      " @ " +
      message.timestamp.local().format("h:mm a") +
      "</strong>:<br>" +
      message.text +
      "</p>"
  );
...
$form.on("submit", function (event) {
  ...
  if (sentMessage !== "") {
    ...
    $messages.append(
      "<p class='col-xs-12 col-sm-8 col-sm-offset-2 message sent'><strong>You @ " +
